...
!      write(*,*)'Basis sets... ', BASIS1, BASIS2 
    ...
    IF(QRESET) THEN 
       ...
       QMHUB=.false.        ! Dr. Pan's QM/MM Interfacer   ! Added by Raafik 2023
...
!           'QCHEM> Q-Chem will solve ',NROOts,' roots.'
    ...
    QMHUB=(INDXA(COMLYN,COMLEN,'QMHB').GT.0)  ! raafik 2023  ! Added variable in ltm/gamess_ltm.F90

!QMHUB RAAFIK 2023
    IF(QMHUB) THEN
      WRITE(OUTU,22) &
               'QMHub Interfacer Selected: Setup qmhub.ini control file in current directory'
    ENDIF
!QMHUB RAAFIK 2023
...
!      ENDIF 
  ...
  SUBROUTINE QCHEM(E,DX,DY,DZ,CGX,AMASSX,IACX,NDD1,DD1,QSECD,IUPT,JUPT)
    ...
    logical :: e_read  ! RAAFIK 2023 QMHUB
          ...
          ELSE         
             ...
             ELSEIF (QSAVORB) THEN 
                ...
                QSAVORB=.FALSE.
                  ...
                  ! /* RAAFIK 2023 */
                ELSEIF (QMHUB) THEN
                  CALL SYSTEM('bash qmhub_inpgen.sh ' // FILIN(1:LI)// ' ; bash qmhub_inpgen_2.sh q11.inp ; $QCHEMEXE -m qmhub -t q12.inp -c ./ qmhub.ini ; bash qmhub_outgen.sh q12.inp q12.out ' //FILOUT(1:LO) )  
                  !CALL SYSTEM('bash qmhub_inpgen.sh ' // FILIN(1:LI)// ' ; bash qmhub_inpgen_2.sh q11.inp ; $QCHEMEXE -m qmhub -t q12.inp -c ./ qmhub.ini ' )  
                  ! /* RAAFIK 2023 */
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!TO MAKE QMHUB SPECIFICS
! if(.not. qmhub) run the above full block
! if(qmhub) run the below block
...
20   if (QMHUB) then
                  ...
                  e_read = .true.
                  if (e_read) then
                    print *, "Energy read successfully. Value of 'e' is:", e*tokcal
                    goto 200 ! Jump to label 200 if e_read is true
                  ENDIF
                  else  
                     ! /* RAAFIK 2023 */
...
!      Make "5" a user setable option.... 
    ...
    IF(QQCRP) THEN
      ...
      ELSEIF(.not.QMHUB) THEN   ! /* RAAFIK 2023 */ ELSE
   ...
   !  /* RAAFIK 2023 */
    ...
    WRITE(outu, 22) 'QMHub MM Forces Updated'
      ...
      DO I=MMATOMS+1,MMATOMS+QMATOMS
         ...
         ENDIF
      ENDDO
      WRITE(outu, 22) 'QMHub QM Forces Updated'
   ELSE
          ! Print a message for the false case
    WRITE(outu, 22) 'Updating forces from QChem direct interface. QMHub Interfacer Not Enabled'
   !  /* RAAFIK 2023 */
...
999       CONTINUE
    ...
    IF(QGMREM) THEN
       DO I=1, NATOM
         CG_DUM(I) = CG(I)   ! raafik 2023 qmhub
          !           Replica ! use ABS()
