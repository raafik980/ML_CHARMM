...
!  common /divch/ ndiv, zlhost
  ...
  LOGICAL QMP2,QLMP2,QCCSD,QCIS,QRIMP2,QSOSMP2,QMOSMP2,QSCSMP2, &
       ...
       QMIXED,QQCSCRATCH,QQCHEM,QQEWALD,QMESS,QQRESD,QQCONS,QQNOLKATMS, QMHUB ! raafik 2023 ! added qmhub
  real(chm_real),allocatable,dimension(:) :: CG_DUM ! raafik 2023 ! added qmhub
...
!   MMALpha=ZERO
  ...
  subroutine allocate_gamess()
    ...
    if(qmused_quantum.or.qmused_qchem.or.qmused_g09.or.qmused_turbo.or. &
       ...
       if(allocated(cg_dum))call chmdealloc(file_name,routine_name,'cg_dum',maxaim,crl=cg_dum) ! /*RAAFIK 2023 */ Added for QMHUB gukini.F90
       ...
       call chmalloc(file_name,routine_name,'cg_dum',maxa  ,crl=cg_dum) ! /*RAAFIK 2023 */ Added for QMHUB gukini.F90
    endif
#if KEY_GAMESSUK==1
